...
void bravo() {
  ...
  int interTime = millis();
  while (findSmallest() > 150 && (totalTime < startTime + 21000) && movestate == 0) {
    Forward();
  }
  while (findSmallest() < 150 && (totalTime < startTime + 21000) && movestate == 0) {
    totalTime = totalTime + (startTime - millis());
    Stop();
  }
  if (totalTime >= startTime + 21000 && movestate == 0) {
    movestate == 1;
    Stop();
  }
}
