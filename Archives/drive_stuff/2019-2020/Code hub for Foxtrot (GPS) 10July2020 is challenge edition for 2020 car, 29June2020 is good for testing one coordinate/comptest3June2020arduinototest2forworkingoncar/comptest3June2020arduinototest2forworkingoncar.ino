...
int leftangle = 81;//for 45 degree angle
int center = 86;
int rightangle = 96; //for 45 degree angle
int TrueSpeed;
int Angle;
int moveport = 9;
int turnport = 10;
int slightleftangle = 82;
int slightrightangle = 93;
int latlong = 0;
...
double TLongitudes[] = {-83.430343, -83.431129, -83.431129, -83.431953};
...
double changebearing(float crse, float brg){
  ...
  } else if (diffheading > 0 && diffheading < 60) {
    slightLeft();
  } else if (diffheading < 0 && diffheading > -60) {
    slightRight();
  ...
  double moving(float Clat, float Clong, float Tlat, float Tlong){
    if((Clat >= Tlat-0.00008&&Clat<=Tlat+0.00008)&&(Clong >= Tlong-0.00008&&Clong<=Tlong+0.00008)){
      colorWipeStrip(strip.Color(  0,   0, 255), 50); // Blue
    colorWipeWheel(wheel.Color(  0,   0, 255), 50); // Blue 
     Serial.println("motorstop");
      if(latlong == 3){
        colorWipeWheel(wheel.Color(  0,   255, 0), 50); // Green 
        colorWipeStrip(strip.Color(  0,   255, 0), 50); // Green 
        Stop();
        }else {
          }
     delay(5000);
...
void printGPSInfo()
{
  ...
  CLatitude = tinyGPS.location.lat(), 10;
  CLongitude = tinyGPS.location.lng(), 10;
