...
double turnheading(float heading, float  bearing){
  int degree = bearing - heading; 
  if (heading == bearing) { 
    return Serial.println("0"); 
  } 
    else if (-60 < degree < 60) {
      return Serial.println(degree); 
    }
    else if(degree > 60){
...
double turncompass(float compass, float bearing){
  int degree = compass - bearing;
  if(compass == bearing){ 
    return Serial.println("0");
  }
    if(-60 < degree < 60) {
...
void loop()
{
 SerialMonitor.print("Lat: "); SerialMonitor.println(tinyGPS.location.lat(), 50);
  ...
  SerialMonitor.print("bearing:");bearing(CLatitude, CLongitude, TLatitude, TLongitude); 
  SerialMonitor.print("compass:");compass();
  SerialMonitor.print("moving:");moving(CLatitude, CLongitude, TLatitude, TLongitude);
  SerialMonitor.print("turnheading:");turnheading(tinyGPS.course.deg(), bearing(CLatitude, CLongitude, TLatitude, TLongitude));
 SerialMonitor.print("turncompass:");turncompass(compass(), bearing(CLatitude, CLongitude, TLatitude, TLongitude));
  delay(1000);
  //SerialMonitor.print("turnto:"); getto(bearing, heading1); 
}

boolean theRange(double z, int x, int y) {
  if (x < z < y){
    return true;
  }
  else {
...
// Normalize to 0-360
  ...
  /*double getto(){
    ...
    else if (-60 < heading1 < 60) {
      return Serial.println(heading1); 
    }
    else if(heading1 > 60){
        return Serial.println("turn 60");
    }
